...
const runCommand = {
  ...
  pathArgs: ['cwd'],
  description:
    'Windows PowerShell komutu calistirir ve ciktisini dondurur. Dosya okuma/yazma icin oncelikle read_file ve write_file araclarini tercih et; bunu sistem sorgulari, program calistirma ve baska aracin karsilamadigi isler icin kullan.',
  parameters: {
    type: 'object',
    properties: {
  ...
  async preview(args, ctx) {
    ...
    return `PowerShell komutu calistirilacak\nKlasor: ${cwd.path}\n\n${args.command}`;
  },
  async handler(args, ctx) {
    const cwd = resolvePath(args.cwd || ctx.config.workspaceRoot, ctx.config.workspaceRoot);
    const res = await runPowerShell(String(args.command), cwd.path, ctx.signal);
